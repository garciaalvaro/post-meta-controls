 ...
 * @since 1.0.0
 */
function create_instances($sidebars_props = [])
{
	$instances = [
		"sidebars" => [],
		"tabs" => [],
	...
	foreach ($sidebars_props as $sidebar_props) {
		...
		foreach ($sidebar_props["tabs"] as $tab_props) {
			...
			$tab_path = [$sidebar_path, $tab->get_id()];
			...
			foreach ($tab_props["panels"] as $panel_props) {
				...
				$panel_path = array_merge($tab_path, [$panel->get_id()]);

				foreach ($panel_props["settings"] as $setting_props) {
					$setting_props = add_root_props(
						$setting_props,
						$panel_path,
						$root_props,
						true
