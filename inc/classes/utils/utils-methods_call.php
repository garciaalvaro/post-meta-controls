 ...
 * @since 1.0.0
 */
function register_meta($setting_instances = [])
{
	$data_key_array = [];
 ...
 * @since 1.0.0
 */
function enqueue_locale($setting_instances = [])
{
	foreach ($setting_instances as $setting_instance) {
 ...
 * @since 1.0.0
 */
function get_props($instances = [], $post_type_current = "")
{
	$props_array = [];
	...
	foreach ($instances as $instance) {
		...
		if (
			empty($post_type) ||
			(is_array($post_type) &&
				in_array($post_type_current, $post_type)) ||
 ...
 * @since 1.0.0
 */
function set_meta_key_exists($setting_instances = [], $post_id = 0)
{
